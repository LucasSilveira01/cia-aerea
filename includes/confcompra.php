...
<?php
    ...
    $cpf_passageiro = $_POST['cpf_person'];
    $cpf_comprador = $_POST['cpf'];
    $nome_passageiro = $_POST['nome_person'];
    $nome_comprador = $_POST['nome'];
    if($_POST['choice-radio'] == "Sim"){
        $passageiro = 0;
        $sql = "insert into cliente(`cpf`, `nome`, `passageiro`) select * from(select $cpf_comprador, '$nome_comprador', $passageiro) as tmp where not exists
        (select cpf from cliente where cpf = $cpf_comprador) limit 1;";
        $sql2 = "insert into passageiro(`cpf`, `nome`) select * from(select $cpf_passageiro, '$nome_passageiro') as tmp where not exists
        (select cpf from passageiro where cpf = $cpf_passageiro) limit 1;";
    ...
    $sql = "select * from voo v join aeroporto a on(v.aeroportopartida = a.codaeroporto) where codvoo = $voo";
    $query = mysqli_query($conn, $sql);
    $dados_partida = mysqli_fetch_array($query);
    $data = array(
        'Acre'=>'AC',
        'Alagoas' =>'AL',
        'Amapá'=> 'AP',
    ...
    $estado_partida =  ($data[$dados_partida['Estado']]);
    $sql = "select * from voo v join aeroporto a on(v.aeroportochegada = a.codaeroporto) where codvoo = $voo";
    $query = mysqli_query($conn, $sql);
    $dados_chegada = mysqli_fetch_array($query);
    $estado_chegada =  ($data[$dados_chegada['Estado']]);
    if($_POST['choice-radio'] == "Sim"){
        $nome = $_POST['nome_person'];
        $cpf_comprador = $_POST['cpf'];
        $cpf_passageiro = $_POST['cpf_person'];
        
        $sql = "insert into ticket(`comprador`, `passageiro`, `codvoo`,`poltrona`) values($cpf_comprador, $cpf_passageiro, $voo, $poltrona)";
    }else{
        $nome = $_POST['nome'];
        $cpf_comprador = $_POST['cpf'];
        $sql = "insert into ticket(`comprador`, `passageiro`, `codvoo`,`poltrona`) values($cpf_comprador, $cpf_comprador, $voo, $poltrona)";
    }
    //echo $sql;
    (mysqli_query($conn, $sql))
?>
<main class="ticket-system">
   <div class="top">
   <h1 class="title">Aguarde 1 segundo, seu bilhete está sendo gerado</h1>
   <div class="printer" />
   </div>
      ...
      <div class="receipts">
         <div class="receipt">
            ...
            <div class="route">
               ...
               <h2><?php echo $estado_chegada;?></h2></h2>
            </div>
            <div class="details">
               <div class="item">
                  <span>Passanger</span>
               ...
               <div class="item">
                  <span>Departure</span>
                  <h3><?php echo $dados_partida['DataPartida']." ".substr($dados_partida['HoraPartida'],0,5); ?></h3>
               </div>
               <div class="item">
                  <span>Seat</span>
                  <h3><?php echo $poltrona;?></h3>
               </div>
            </div>
         </div>
